...
const HomePage = () => {
  ...
  const products = [
    { title: 'iPhone 13 Pro Max', price: '₦450,000', store: 'TechHub Store', negotiable: true },
    { title: 'iPhone 13 Pro Max', price: '₦450,000', store: 'TechHub Store', negotiable: false },
  ];
  ...
  return (
    <>
      ...
      <div className="w-full min-h-screen relative flex flex-col py-6 pb-10">
        {/* ── Nav ── */}
        ...
        <div className="px-5 pt-5 mb-8">
          <h1 className="text-text-primary text-2xl font-semibold leading-tight">Olamilekan</h1>
          <p className="text-text-secondary text-sm mt-0.5">Welcome to TradeNG.</p>
        </div>
        ...
        <div className="px-5 mb-5">
          <div className="flex gap-3 overflow-x-auto no-scrollbar pb-1">
            {categories.map(cat => (
              <div key={cat.label} className="flex flex-col flex-shrink-0 w-[74px] h-[78px] rounded-[12px] border border-gray-200 items-center justify-center text-[30px]">
                {cat.emoji}
                <span className="text-[#374151] text-[10px] font-medium">{cat.label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="px-5 mb-5">
          <div
            ...
            style={{ background: 'linear-gradient(100deg, #0D0500 0%, #7A1E00 45%, #C03300 100%)' }}
          >
            <div className="flex items-center justify-between px-5 py-5">
              <div>
                <p className="text-white text-[15px] font-bold leading-snug">Buy Safely with Escrow</p>
                <p className="text-white/65 text-xs mt-1">Your money is protected until delivery</p>
              </div>
        ...
        <div className="px-5 mb-2">
          <div className="flex items-center justify-between mb-3.5">
            <h2 className="text-text-primary text-sm font-medium">Featured Item</h2>
            <button className="text-text-secondary text-xs">View All</button>
          </div>

          <div className="flex gap-3">
            {products.map((p, i) => (
              <ProductCard key={i} {...p} onClick={() => router.push('/item-detail')} />
            ))}
          </div>
        </div>
